<?php
    class Member_Model extends MY_Model {
        ...
        */
        ...
        {
            $where = $data['where'];
            $sql = "SELECT count(*) cnt FROM tb_user";
        ...
        {
            $sql = "SELECT count(*) cnt FROM tb_user users WHERE (user_status = 'Y' OR user_status = 'C' OR user_status = 'R') {$where}";
            $result = $this->db->query($sql)->row_array();
        ...
        public function getDayMemberInsight($year,$month,$where="")
        {
            ...
            $sql = "SELECT DAY(reg_date) day,count(*) cnt FROM tb_user users WHERE (user_status = 'C' OR user_status = 'R' OR user_status = 'Y') AND YEAR(reg_date) = '{$year}' AND MONTH(reg_date) = '{$month}' {$where} GROUP BY DATE(reg_date) ORDER BY DATE(reg_date) ASC";
            $day_total = $this->db->query($sql)->result_array();

            $sql = "SELECT count(*) cnt FROM tb_user users WHERE (user_status = 'C' OR user_status = 'R' OR user_status = 'Y') AND DATE(reg_date) <= '{$last_day}' {$where}";
            
            $all_total = $this->db->query($sql)->row_array();

            $sql = "SELECT count(*) cnt FROM tb_user users WHERE user_status = 'N' AND YEAR(withdrawal_date) = '{$year}' AND MONTH(withdrawal_date) = '{$month}' {$where}";
        ...
        public function getSearchLeaveMemberTotal($year,$month,$where="")
        {
            if($month=="all"){
                $sql = "SELECT count(*) cnt FROM tb_user users
                                WHERE (user_status = 'N')  AND YEAR(withdrawal_date) = '{$year}' {$where}";
            }else{
                $sql = "SELECT count(*) cnt FROM tb_user users
                                WHERE (user_status = 'N') AND YEAR(withdrawal_date) = '{$year}' AND MONTH(withdrawal_date) >= '{$month}' {$where}";
            }
