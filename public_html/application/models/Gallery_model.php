<?php
  class Gallery_Model extends CI_Model {
    ...
    //get news
    public function getGallery($seq)
    {
      $query = "SELECT * FROM tb_board_gallery WHERE board_seq = '{$seq}'";
    ...
    //get boards total
    public function getGalleryTotalCount($data)
    {
      $query = "SELECT count(*) cnt FROM tb_board_gallery WHERE board_view_yn = 'Y' AND board_display_yn = 'Y'";
    ...
    public function getGalleryList($data)
    {
      ...
      $query = "SELECT * FROM tb_board_gallery WHERE board_view_yn = 'Y' AND board_display_yn = 'Y' ORDER BY board_seq DESC {$limit}";
    ...
    {
      $query = "SELECT board_read_cnt FROM tb_board_gallery WHERE board_seq = '{$seq}'";
      $row = $this->db->query($query)->row_array();

      $num = (int)$row['board_read_cnt'];
      $num += 1;

      $query = "UPDATE tb_board_gallery SET board_read_cnt = '{$num}' WHERE board_seq = '{$seq}'";
