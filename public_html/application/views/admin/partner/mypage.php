                ...
                <table class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <th>소속명</th>
                      <td colspan="3">
                        <input type="hidden" class="form-control col-sm-6 float-left" id="group_name" name="group_name" placeholder="소속명" value="<?php echo @$data['group_name'];?>" required />
                    ...
                    <tr>
                      <th>휴대폰번호</th>
                      <td colspan="3">
                        <input type="text" class="form-control col-sm-6 float-left" id="cell_no" name="cell_no" placeholder="휴대폰번호" value="<?php echo @$data['cell_no'];?>"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required/>
                      </td>
                    </tr>
                    <tr>
                      <th>이메일</th>
                    ...
                    <tr>
                      <th>이용 상품</th>
                      <td colspan="3">
                        <select class="form-control col-sm-4 float-left" name="pricing_plan" id="pricing_plan" style="display:none">
                            <?php 
                            for($i=0;$i<count($planList);$i++) {
                                if($data['pricing_plan']==$planList[$i]['code_type'])
                                    $pricing_plan = $planList[$i]['code_name'];
                                ?>
                                <option value="<?php echo $planList[$i]['code_type'];?>" <?php echo @$data['pricing_plan']==$planList[$i]['code_type']?"selected":"";?>><?php echo $planList[$i]['code_name'];?></option>
                            <?php }?>
                        </select>
                    ...
                    <tr>
                      <th>이용금액</th>
                      <td colspan="3">
                        <input type="hidden" class="form-control col-sm-6 float-left" id="pricing_price" name="pricing_price" placeholder="이용금액" value="<?php echo @$data['pricing_price'];?>" required />
                        <?php echo @$data['pricing_price'];?>
                      </td>
                    </tr>
                    <tr>
                      <th>사업자번호</th>
                      <td colspan="3">
                        <input type="hidden" class="form-control col-sm-6 float-left" id="business_no" name="business_no" placeholder="사업자번호" value="<?php echo @$data['business_no'];?>" required />
                        <?php echo @$data['business_no'];?>
                      </td>
                    </tr>                    
                    <tr>
                      <th>계약일</th>
                    ...
                    <tr>
                      <th>사용인원</th>
                      <td colspan="3">
                        <input type="hidden" class="form-control col-sm-6 float-left" id="use_count" name="use_count" placeholder="사용인원" value="<?php echo @$data['use_count'];?>"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/>
                        <?php echo @$data['use_count'];?>
                      </td>
                    </tr>                    
                    <tr>
                      <th>마케팅 수신동의</th>
                      <td colspan="3">
                        <?php echo @$data['sms_yn'];?>
                      </td>
                    </tr>
                    <tr>
                      <th>개인정보 이용동의</th>
                      <td colspan="3">
                        <?php echo @$data['marketing_yn'];?>
                      </td>
                    </tr>
                    <tr>
                      <th>로고 등록</th>
                      <td colspan="3">
                        <input type="file" class="form-control col-sm-6 float-left" id="file" name="file" placeholder="로고"  onchange="uploadImg($(this), this, '')"  data-target="logo_data" data-seq="1" data-type="image"  data-maxsize="100">
                        <div id="logo_data"><img src='/upload/logo/<?php echo @$data['logo']?>'></div>
                        <p style="clear:both;color:red;margin:0px;">*로고 이미지는 포트폴리오에만 노출됩니다.</p>
                        <p style="margin:0px;">*이미지는 400KB 미만, 720*350(px), PNG 파일만 등록 가능</p>
                        
                        <input type="hidden" name="logo" id="logo" value="<?php echo @$data['logo']?>" /> 
                      </td>
                    </tr> 
                  </tbody>
                </table>
